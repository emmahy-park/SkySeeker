...
/*
	Loads the textures based on the paths into the handles. Handles should have enough space to hold num_textures.

	for a vector this is achieved by initializing it with a number of default values.
	i.e. vector<GLuint> handles = vector<GLuint>(num_textures, 0)  
*/
void RenderSystem::loadGlTextures(GLuint* handles, const std::string* texture_paths, size_t num_textures) {
	glGenTextures((GLsizei) num_textures, handles);

	for (uint i = 0; i < num_textures; i++)
	{
		const std::string& path = texture_paths[i];
		ivec2 dimensions; // We can add this if we need dimensions. For now ignored...
...
void RenderSystem::unloadMapTilesets(GLuint* handles, int num_handles) {
	glDeleteTextures((GLsizei)num_handles, handles);
}
...
bool RenderSystem::initTexturedQuad() {
	...
	std::string path = textures_path("startscreen/startscreen.png");
	loadGlTextures(&quad_texture, &path, 1);

	return true;
}

bool RenderSystem::particleInit() {
	// enable blending or you will just get solid boxes instead of text
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

	// buffer setup
	glGenBuffers(1, &particle_transform_VBO);
	glBindBuffer(GL_ARRAY_BUFFER, particle_transform_VBO);

	return true;
}

bool RenderSystem::fontInit(GLFWwindow* window) {
	// enable blending or you will just get solid boxes instead of text
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	std::vector<std::pair<std::string, std::string>> fonts = {
		{"KnightWarrior", font_path("KnightWarrior.otf")},
		{"KenneyPixel", font_path("Kenney_Pixel_Square.ttf")}
	};

	// disable byte-alignment restriction in OpenGL
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

	for (const auto& [fontName, fontPath] : fonts) {
		FT_Face face;
		...
		// Set font size
		FT_Set_Pixel_Sizes(face, 0, fontName == "KnightWarrior" ? 50 : 40);
		...
		m_fonts.insert({fontName, characterMap});
...
bool RenderSystem::initImGui(GLFWwindow* window) {
	IMGUI_CHECKVERSION();
	ImGui::CreateContext();
	...
	std::string Kenney_Pixel = font_path("Kenney_Pixel_Square.ttf");

	bigFont = io.Fonts->AddFontFromFileTTF(Kenney_Pixel.c_str(), 30.0f);
	medFont = io.Fonts->AddFontFromFileTTF(Kenney_Pixel.c_str(), 18.0f);
	smallFont = io.Fonts->AddFontFromFileTTF(Kenney_Pixel.c_str(), 15.0f);

	io.FontDefault = medFont;
...
{
	loadGlTextures(texture_gl_handles.data(), texture_paths.data(), texture_gl_handles.size());
	gl_has_errors();
}
...
RenderSystem::~RenderSystem()
{
	...
	while (registry.textureinfos.entities.size() > 0) {
		registry.remove_all_components_of(registry.textureinfos.entities.back());
	}
}
...
void RenderSystem::initializeShadows() {
	// Inspired by https://learnopengl.com/Advanced-Lighting/Shadows/Shadow-Mapping, which changes for a 2D scene
	...
	glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, lightShadowMap, 0);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	gl_has_errors();
